...
import { useRef, useCallback, useEffect, type PointerEvent, type ReactNode } from 'react';
...
interface GlowBorderProps {
  ...
  borderRadius?: number;
  radius?: number; // Legacy compat
  glowRadius?: number;
  spread?: number; // Legacy compat
  glowIntensity?: number;
  coneSpread?: number;
  animated?: boolean;
  sweepOnMount?: boolean; // Legacy compat
  colors?: string[];
  fillOpacity?: number;
  inline?: boolean;
}
...
export function GlowBorder({
  ...
  className = '',
  edgeSensitivity = 70,
  glowColor = '217 91 60', // Using your cool blue tone
  ...
  const actualGlowRadius = glowRadius ?? spread;
  const actualAnimated = animated ?? sweepOnMount;
  ...
  useEffect(() => {
    if (!actualAnimated || !cardRef.current) return;
    const card = cardRef.current;
  ...
  return (
    <div
      ...
      className={`glow-border-card${lightSurface ? ' glow-border-card--light' : ''} ${inline ? ' glow-border-card--inline' : ''} ${className}`}
      style={{
        '--card-bg': backgroundColor,
        '--edge-sensitivity': edgeSensitivity,
        '--border-radius': `${actualRadius}px`,
        '--glow-padding': `${actualGlowRadius}px`,
